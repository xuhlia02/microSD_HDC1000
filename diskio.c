...
void power_on (void)
{
   SPI_PORT = (1<<DD_SSAD) | (1<<DD_SSSD) | (1<<DD_MISO);
   SPI_DDR  |=  (1<<DD_SSAD) | (1<<DD_SSSD);
   SPI_DDR &= ~ (1<<DD_MISO); 
   SPI_DDR |= (1<<DD_MOSI) | (1<<DD_SCK) | (1<<DD_SSSD);
    
   //nastavenie funkcneho registra SPI (SPCR) 
...
)
{
	...
	do {							/* Wait for data packet in timeout of 200ms */
		token = rcvr_spi(); Timer1--; _delay_ms(10);
	...
	if (send_cmd(CMD0, 0) == 1) {			/* Enter Idle state */
		...
		if (send_cmd(CMD8, 0x1AA) == 1) {	/* SDv2? */
			...
			if (ocr[2] == 0x01 && ocr[3] == 0xAA) {				/* The card can work at vdd range of 2.7-3.6V */
				for (Timer1 = 100; Timer1 && send_cmd(ACMD41, 1UL << 30); Timer1--) _delay_ms(10);	/* Wait for leaving idle state (ACMD41 with HCS bit) */
			...
			for (Timer1 = 100; Timer1 && send_cmd(cmd, 0); Timer1--) _delay_ms(10);	/* Wait for leaving idle state */
